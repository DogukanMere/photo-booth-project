...
const EventItem = (props) => {
  const { header, text, speech, iconReact } = props;
  return (
    <div className='grid-span-1 bg-paper h-full rounded-2xl px-5 pt-10 pb-[60px] flex flex-col justify-start items-center'>
      <div className='mt-5 min-w-[6rem] h-[6rem] sm:min-w-[10rem] sm:h-[10rem] bg-secondaryLight rounded-full overflow-hidden flex justify-center items-center'>{iconReact}</div>
      <div className='flex flex-col gap-y-5 text-pretty'>
        ...
        <i className='text-grey600 px-7 md:px-10 leading-9 self-end justify-self-end '> - {speech}</i>
      </div>
    </div>
  );
};
